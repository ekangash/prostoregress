...
for domain in ${!domains[*]}; do
  ...
  if [ ! -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    -days 1 -keyout '$path/privkey.pem' -out '$path/fullchain.pem' -subj '/CN=localhost'" letsencrypt-certbot
  fi

done

echo "### Starting letsencrypt-nginx-webroot container for access domain temporary files..."
# Restarting for case if letsencrypt-nginx-webroot container is already started
docker-compose up -d letsencrypt-nginx-webroot && docker-compose restart letsencrypt-nginx-webroot
...
for domain in ${!domains[*]}; do
  ...
  if [ -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    $staging_arg $email_arg --rsa-key-size $rsa_key_size --agree-tos --force-renewal --non-interactive" letsencrypt-certbot
  fi
done
